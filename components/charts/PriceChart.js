...
function formatDate(dateStr, days) {
  ...
  if (days <= 7) return date.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit' })
...
export default function PriceChart({ symbol, assetType, analyses = [] }) {
  ...
  async function fetchData() {
    ...
    if (prices.length > 0) {
      ...
      const p20 = first + (max - first) * 0.2
      const p80 = first + (max - first) * 0.8
  ...
  function buildChartData() {
    ...
    return data.map(row => {
      ...
      const analysisOnDate = analyses.find(a =>
        a.created_at?.split('T')[0] === row.date
      )
      return {
        date: row.date,
        price: close,
        ...
        <LineChart data={chartData} margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
          ...
          {(activeChartType === 'average' || isSPY) && (
            <>
              ...
              {stats && (
                <ReferenceLine y={stats.avg} stroke="var(--text-tertiary)" strokeDasharray="4 2" strokeWidth={1.5} />
              )}
            </>
          )}
