...
export function callOpus(prompt) {
  return callClaude('claude-opus-4-6', 2000, prompt)
}

export async function sendPushNotification({ title, body, assetSymbol, triggeredBy = 'automatic', userId = null }) {
  ...
  const notifUrl = notif
    ? `/?tab=notifications&notifId=${notif.id}`
    : '/?tab=notifications'
  ...
  for (const sub of subscriptions || []) {
    try {
      await webpush.sendNotification(
        ...
        JSON.stringify({ title, body, url: notifUrl, notifId: notif?.id })
      )
    } catch (err) {
