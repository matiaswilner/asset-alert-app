...
function buildPortfolioContext(position) {
  ...
  const isUnderwater = currentPrice < avgCost

  let concentrationSignal
  if (weight > 20) concentrationSignal = 'ALTA (>20% del portfolio) — ser muy conservador con nuevas compras'
  else if (weight > 10) concentrationSignal = 'MODERADA (10-20% del portfolio) — evaluar cuidadosamente antes de agregar'
  else concentrationSignal = 'BAJA (<10% del portfolio) — hay margen para acumulación gradual'

  return `
Portfolio position:
- Quantity: ${parseFloat(position.quantity).toFixed(4)} shares
- Average cost: $${avgCost.toFixed(2)}
- Current price vs avg cost: ${isUnderwater ? `BAJO agua (${pnlPct.toFixed(2)}%)` : `EN ganancia (+${pnlPct.toFixed(2)}%)`}
- Unrealized P&L: $${unrealizedPnl.toFixed(2)}
- Portfolio weight: ${weight.toFixed(2)}%
- Concentration signal: ${concentrationSignal}
`
}

export function buildAnalysisPrompt({ symbol, priceChange, timeframe, newsData, currentPrice, previousPrice, portfolioPosition = null }) {
  const sectorContext = SECTOR_CONTEXT[symbol] || 'Stock or asset'
  const portfolioContext = buildPortfolioContext(portfolioPosition)
...
${portfolioContext ? `   IMPORTANT: The user already owns this asset. Consider their current position, average cost, and portfolio weight when making the recommendation.` : ''}
6. Assign a score from -5 to +5:
   - Negative = not a good time to buy
   - Zero = neutral / wait
   - Positive = potential gradual buying opportunity
${portfolioContext ? `   IMPORTANT: If the asset weight is >20%, be more conservative with positive scores.` : ''}
